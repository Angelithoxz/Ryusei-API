...
function checkKey(req, res) {
  ...
  db.prepare(`
    UPDATE users SET max_requests = max_requests - 1 WHERE apikey = ?
  `).run(apikey);
  user.max_limit--;
  return user;
}
...
function updateUser(username, field, value) {
  ...
  console.log(`Actualizando usuario: ${username}, campo: ${field}, valor: ${value}`);
  const stmt = db.prepare(`UPDATE users SET ${field} = ? WHERE username = ?`);
  const result = stmt.run(value, username);
